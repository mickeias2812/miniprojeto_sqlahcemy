...
def adicionar_livro():
    ...
    # Verificar se há autores, gêneros e editoras cadastrados
    if not autores:
        flash("É necessário cadastrar pelo menos um autor antes de adicionar livros!", "error")
        cursor.close()
        conn.close()
        return redirect(url_for('listar_livros'))
    
    if not generos:
        flash("É necessário cadastrar pelo menos um gênero antes de adicionar livros!", "error")
        cursor.close()
        conn.close()
        return redirect(url_for('listar_livros'))
    
    if not editoras:
        flash("É necessário cadastrar pelo menos uma editora antes de adicionar livros!", "error")
        cursor.close()
        conn.close()
        return redirect(url_for('listar_livros'))
...
def adicionar_emprestimo():
    ...
    cursor.execute("SELECT * FROM Usuarios")
    usuarios = cursor.fetchall()
    cursor.execute("SELECT * FROM Livros WHERE Quantidade_disponivel > 0")  # Só livros disponíveis
    livros = cursor.fetchall()
    cursor.close()
    conn.close()
...
def editar_emprestimo(id):
    # Busca dados do empréstimo, usuários e livros disponíveis
    conn = get_connection()
    cursor = conn.cursor(dictionary=True)
    ...
    if request.method == 'POST':
        ...
        cursor.close()
        conn.close()
        flash("Empréstimo atualizado!", "ssuccess")
        return redirect(url_for('listar_emprestimos'))
